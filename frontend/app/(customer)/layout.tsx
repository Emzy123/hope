  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50/50">
        ...
        <span className="text-xs font-black text-slate-500 mt-3 uppercase tracking-widest">Verifying Customer Session...</span>
      </div>
    );
  }

  if (!user || user.role.toLowerCase() !== "customer" || !user.is_onboarded) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50/50">
      ...
      <footer className="bg-white border-t border-slate-100 py-6 text-center text-xs font-black text-slate-400">
        SkillBridge Nigeria Customer Center © 2026.
      </footer>
    </div>
  );
}
